...
function EditorApp() {
  ...
  const searchResults = useMemo(() => {
    ...
    functions.forEach((fn) => {
      ...
      ;[
        ...
        ['Descrição da função', fn.description],
  ...
  function exportToExcel() {
    ...
    const personSummarySheet = people.flatMap((person) => {
      ...
      if (!data.responsibleAreas.length && !data.responsibleFunctions.length && !data.involvedItems.length) {
        rows.push({
          Pessoa: person.name,
          Tipo: 'Sem dados',
          Contexto_1: '',
      ...
      data.responsibleAreas.forEach((row) => {
        rows.push({
          Pessoa: person.name,
          Tipo: 'Área',
          Contexto_1: row.areaName,
          Contexto_2: '',
          Papel: row.role,
      ...
      data.responsibleFunctions.forEach((row) => {
        rows.push({
          Pessoa: person.name,
          Tipo: 'Função',
          Contexto_1: row.areaName,
        ...
        <div style={boxStyle}>
          ...
          <div style={{ display: 'grid', gap: 6 }}>
            {filteredProjects.map((project) => {
              ...
              const projectTasks = selectedProjectId === project.id ? selectedProjectTasks : []
          ...
          {workItemForm ? (
            <div style={boxStyle}>
              <div style={{ display: 'grid', gap: 10 }}>
                ...
                <label>Descrição<textarea style={{ ...inputStyle, minHeight: 80 }} value={workItemForm.description ?? ''} onChange={(e) => setWorkItemForm((s) => ({ ...s, description: e.target.value }))} /></label>
