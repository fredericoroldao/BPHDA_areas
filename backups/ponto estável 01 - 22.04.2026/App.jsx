...
function taskState(task, dependenciesBySuccessor, workItemMap) {
  ...
  return unresolved ? 'à espera de dependências' : 'pronta a começar'
}
...
export default function App() {
  ...
  function togglePersonProject(personId, projectName) {
    ...
    setCollapsedPersonProjects((prev) => ({ ...prev, [key]: !prev[key] }))
  }
  ...
  function isPersonTaskCollapsed(personId, taskId) {
    return !!collapsedPersonTasks[`${personId}:${taskId}`]
  }
          ...
          <div style={{ ...cardStyle(), width: 250, minWidth: 250, maxWidth: 250 }}>
            <div style={{ display: 'grid', gap: 6 }}>
              {peopleData.map((person) => (
                <button
                  key={person.id}
                  style={{ padding:'8px 10px', borderRadius:8, border:'1px solid #b8c9b5', background:openPersonIds.includes(person.id)?'#dcefd8':'#fff', color:'#16361f', textAlign:'left', cursor:'pointer', fontWeight:600 }}
                                              ...
                                              <>
                                                {task.incoming?.length ? (
                                                  <div style={{ marginTop:8 }}>
                                                    <div style={{ fontSize:11, fontWeight:800, color:'#2f4b35', marginBottom:4, textTransform:'uppercase' }}>Tarefa bloqueada por:</div>
                                                    {task.incoming.map(({ dep, item }) => (
                                                      <div key={dep.id} style={{ fontSize:12, marginBottom:4, color:'#16361f' }}>
                                                ...
                                                {task.outgoing?.length ? (
                                                  <div style={{ marginTop:8 }}>
                                                    <div style={{ fontSize:11, fontWeight:800, color:'#2f4b35', marginBottom:4, textTransform:'uppercase' }}>O que esta tarefa bloqueia:</div>
      ...
      <div style={{ minHeight:'100vh', background:'#f2f5ef', padding:'24px 18px 36px' }}>
        <div style={{ maxWidth:1800, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', gap:10, marginBottom:14, flexWrap:'wrap' }}>
            <button onClick={() => setActiveView('areas')} style={{ padding:'8px 12px', borderRadius:999, border: activeView === 'areas' ? '1px solid #6e9575' : '1px solid #b8c9b5', background: activeView === 'areas' ? '#dcefd8' : '#fff', color:'#16361f', cursor:'pointer', fontSize:13, fontWeight:700 }}>Ver por áreas</button>
            <button onClick={() => setActiveView('people')} style={{ padding:'8px 12px', borderRadius:999, border: activeView === 'people' ? '1px solid #6e9575' : '1px solid #b8c9b5', background: activeView === 'people' ? '#dcefd8' : '#fff', color:'#16361f', cursor:'pointer', fontSize:13, fontWeight:700 }}>Ver por pessoa</button>
