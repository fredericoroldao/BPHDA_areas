  ...
  function projectPendingLabel(projectId) {
    ...
    return pending
      .map((item, index) => {
        ...
        const line = [`[${item.name}]`, responsible, due].filter(Boolean).join(' ')
        return `${index + 1}. ${line}`
      })
      .join('\n')
  }
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', gap:8, alignItems:'flex-start' }}>
                    <div style={{ display:'grid', gap:3, minWidth:0 }}>
                      <div style={{ fontSize:12.5, color:'#16361f', lineHeight:1.35, fontWeight:600, wordBreak:'break-word' }}>
                        [{renderHighlightedText(item.name, search)}]
                      </div>
                    </div>
                    ...
                    <div style={{ display:'grid', gap:2, minWidth:0 }}>
                      {responsibleName ? (
                        <div style={{ fontSize:11, color:'#35513c', lineHeight:1.35 }}>
                          <strong>Responsável:</strong> {renderHighlightedText(responsibleName, search)}
                        </div>
                      ) : null}
                      ...
                      <div style={{ border:'1px solid #cfd9ca', borderRadius:12, padding:12, marginBottom:12, background:'#fff' }}>
                        ...
                        {!functionsCollapsed ? (
                          <div style={{ display:'grid', gap:4 }}>
                            {person.functions.map((item, i) => <div key={i} style={{ fontSize:12, color:'#16361f' }}>{renderHighlightedText(item, search)}</div>)}
                          </div>
                        ) : null}
                      </div>
                    ) : null}
                                            ...
                                            <div style={{ display:'grid', gap:6 }}>
                                              <div style={{ fontSize:13, lineHeight:1.35, color:'#16361f', fontWeight:600 }}>
                                                {task.isProjectAssignment
                                                  ...
                                                  : <>Tarefa: {renderHighlightedText(task.label, search)}</>}
                                              </div>
